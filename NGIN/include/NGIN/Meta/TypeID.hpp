...
namespace NGIN::Meta
{
    ...
    struct TypeIDResolver
    {
...

        {
            ...
            (void)dummyVariable; // Use the variable to prevent unused variable
        }
    };
    ...
    */
    ...
    /// \brief Calculates the CRC32 checksum of a string.
    /// CRC stands for Cyclic Redundancy Check.
    /// CRC is mainly used for error detection, but can also act as a hash function.
    /// \details This function uses the zlib polynomial.
    /// \param str The string to calculate the checksum for.
    /// \return The CRC32 checksum of the string.
    [[nodiscard]] constexpr UInt32 CRC32(std::string_view str)
    {
    ...
    template< typename ResultType >
    [[nodiscard]] constexpr ResultType FNV1a(StringView str) noexcept
    {
        ResultType hash = FNV1Traits<ResultType>::basis;
        for (auto c : str)
            hash = (hash ^ c) * FNV1Traits<ResultType>::prime;
        return hash;
    }

    template< typename ResultType >
    [[nodiscard]] constexpr ResultType FNV1(StringView str) noexcept
    {
        ResultType hash = FNV1Traits<ResultType>::basis;
        for (auto c : str)
            hash = (hash * FNV1Traits<ResultType>::prime) ^ c;
        return hash;
    }

    template <typename T>
    [[nodiscard]] constexpr TypeIDType TypeID() noexcept
    {
        return FNV1a<TypeIDType>(TypeName<T>::Full());
    }
